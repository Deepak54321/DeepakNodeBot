...
if(req.body.result.action=='ask')
  ...
  if(req.body.result.action=='testask')
  {
       var result=req.body.result;
            var context=result.contexts[0];
            var PhoneNumber=context.parameters.testphonenumber;
            var Name=context.parameters.TestName;
            var Email=context.parameters.TestEmail;
            var pincode=context.parameters.TestPincode;
            var pattern = /^\d{10}$/;
      console.log("%s",PhoneNumber);
      if(pattern.test(PhoneNumber))
      {
  ...
  if(req.body.result.action=='compask')
  {
       var result=req.body.result;
            var context=result.contexts[0];
            var PhoneNumber=context.parameters.complaintphonenumber;
            var Name=context.parameters.complaintname;
            var Email=context.parameters.ComplaintEmail;
           // var pincode=context.parameters.pincode
            var pattern = /^\d{10}$/;
      console.log("%s",PhoneNumber);
      if(pattern.test(PhoneNumber))
